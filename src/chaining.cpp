...
chains get_chaining(const std::string& str) {
    ...
    std::string gls = "уеэоаы€юиЄ”≈Ёќјџяё»®";
    std::string sgls = "гйцкншщзхждлрпвфчсмтб√…÷ ЌЎў«’∆ƒЋ–ѕ¬‘„—ћ“Ѕ";

    std::string st = str, slv;

    while (st.find_first_of("№ьЏъ") != std::string::npos)
        st.erase(st.find_first_of("№ьЏъ"), 1);


    while (st.length() != 0) {

        while (st.find_first_of(" ,.-:") == 0) //
            st.erase(0, 1);                //убираем знаки препинани€ перед словом

        slv = st.substr(0, st.find_first_of(" ,.-:")); //выдел€ем слово в переменную slv
        st.erase(0, st.find_first_of(" ,.-:"));   // удал€ем слово из строки
